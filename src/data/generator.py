...
class MarinePumpVibrationDataGenerator:
    ...
    def save_signal_to_csv(self, signals: np.ndarray, filename: str = 'normal_vibration.csv') -> None:
        """
    ...
    def plot_signal(self, axes: np.ndarray, signals: np.ndarray, pos: int, title: str = 'Normal pump vibration') -> None:
        """
    ...
    def add_cavitation_effect(self, signals: np.ndarray, sample_rate, severity: float = 'mild', cavitation_start: float = 0.5) -> np.ndarray:
        """
        ...
        num_spikes = int(duration_after_cavitation * spikes_per_second.get(severity, 10))
        ...
        if num_spikes > 0:
            ...
            spike_amp = spike_amplitudes.get(severity, 1.0) * np.random.randn(len(spike_indices))
        ...
        fft_cav = np.abs(np.fft.fft(cavitation_signal[start_index: start_index + 1000])) # Energy (magnitude)
        ...
        log.log_info(f"RMS before cavitation: {rms_before:.3f}, RMS after cavitation: {rms_after:.3f}")
    ...
    def add_ship_motion(self, signals: np.ndarray, marine_condition: str = 'moderate', include_engine_load: bool = True) -> np.ndarray:
        """
        ...
        if include_engine_load:
            ...
            load_profile[phase_duration:2*phase_duration] *= np.random.uniform(0.8, 1.0) # Cruising
            load_profile[2*phase_duration:3*phase_duration] *= np.random.uniform(1.0, 1.2) # Full speed
            load_profile[3*phase_duration:] *= np.random.uniform(0.7, 0.9) # Slowing down
        ...
        bubble_noise = 0.02 * np.random.normal(size=signals.shape) * density_factor * temperature_factor
    ...
    def generate_marine_scenarios(self, base_signal: np.ndarray, including_engine_load: bool = True) -> dict:
        """
        ...
        for scenario in scenarios:
            marine_signal = self.add_ship_motion(signals=base_signal, marine_condition=scenario, include_engine_load=including_engine_load)
    ...
    def generate_dataset(self, n_samples: int = 1000, include_marine_conditions: bool = True, save_to_disk: bool = True) -> tuple: 
        """
        ...
        for i in range(n_samples):
            ...
            severity = np.random.choice(cavitation_severities) if is_cavitation else 'none'
            cavitation_start = np.random.uniform(0.2, 0.7) if is_cavitation else 1.0
            marine_condition = np.random.choice(marine_conditions) if include_marine_conditions else 'land'

            base_signal = self.generate_vibration_signal()
            signals = np.zeros_like(base_signal)

            if is_cavitation:
                signals = self.add_cavitation_effect(signals=base_signal, sample_rate=1000,
                                           severity=severity, cavitation_start=cavitation_start)
            else:
                signals = base_signal

            if include_marine_conditions and  marine_condition != 'land':
                signals = self.add_ship_motion(signals=base_signal, 
            ...
            metadata.append({
                ...
                'cavitation_start_time': cavitation_start if is_cavitation else None,
                'marine_condition': marine_condition,
                'is_marine': include_marine_conditions and marine_condition != 'land',
    ...
    def print_dataset_summarry(self, X_raw: np.ndarray, y: np.ndarray, metadata: list) -> None:
