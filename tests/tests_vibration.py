...
def test_add_cavitation_effect(baseline_vibration) -> None:
    ...
    for severity in severities:
        ...
        assert isinstance(
            cav_signal, np.ndarray
        ), "cavitation signal is not of np.ndarray"
        assert len(cav_signal) == len(
            baseline_signal
        ), "cavitation signal is not as the normal vibration"
...
def test_add_ship_motion(baseline_vibration) -> None:
    ...
    for condition in conditions:
        marine_signal = generator_obj.add_ship_motion(
            baseline_signal, marine_condition=condition, include_engine_load=False
        )

        assert isinstance(marine_signal, np.ndarray), "signals is not of np.ndarray"
        assert len(marine_signal) == len(
            baseline_signal
        ), "marine signal is not as the normal vibration"

        assert not np.array_equal(
            marine_signal, baseline_signal
        ), "marine signal has different shape as normal vibration"
...
def test_generate_dataset_small(baseline_vibration) -> None:
    ...
    X_raw, X_features, y, metadata = generator_obj.generate_dataset(
        n_samples=20, include_marine_conditions=True, save_to_disk=True
    )
...
def test_generate_dataset_large(baseline_vibration):
    ...
    """
    _, generator_obj, _ = baseline_vibration
    X_raw, X_features, _, metadata = generator_obj.generate_dataset(
...
def test_generate_dataset_land_only(baseline_vibration):
    ...
    """
    _, generator_obj, _ = baseline_vibration
    _, _, _, metadata = generator_obj.generate_dataset(
        n_samples=10, include_marine_conditions=False, save_to_disk=False
    )
    for meta in metadata:
...
def test_generate_marine_scenarios(baseline_vibration):
    ...
    expected_scenarios = generator_obj.generate_marine_scenarios(
        normal_vibaration_signal
    )
