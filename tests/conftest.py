...
def baseline_vibration() -> list:
    ...
    threshold = {"mean": 0.1, "std": 0, "rms": 0, "peak": 3, "kurtosis": 1e-6}
    ...
    assert (
        baseline_stats["std"] > threshold["std"]
    ), "Standard deviation should be positive"
    assert baseline_stats["rms"] > threshold["rms"], "RMS value should be positive"
    assert (
        baseline_stats["peak"] < threshold["peak"] * baseline_stats["rms"]
    ), "Peak value must be no exceed 2 - 3 * rms"
